...
This module provides the foundation for collecting player projections from various sources.
"""
        ...
        for attempt in range(self.config.max_retries):
            try:
                ...
                if attempt == self.config.max_retries - 1:
                    raise DataCollectionError(f"API request failed after {self.config.max_retries} attempts: {e}")
...
class BaseWebScrapingCollector(BaseDataCollector):
    ...
    def __init__(self, config: DataCollectionConfig):
        ...
        if config.source_type != DataSourceType.WEB_SCRAPING:
            raise ValueError("Config source_type must be WEB_SCRAPING for WebScrapingCollector")
    ...
    async def collect_from_all_sources(
        ...
        for name, collector in self.collectors.items():
            try:
                ...
                self.logger.info(f"Collected {len(projections)} projections from {name}")
            except Exception as e:
    ...
    async def get_consensus_projections(
        ...
        for (player_id, player_name), projections in player_projections.items():
            if len(projections) >= min_sources:
                ...
                avg_points = sum(p.projected_points or 0 for p in projections) / len(projections)
                ...
                consensus_projection = PlayerProjection(
                    ...
                    projected_value=round(avg_points / avg_salary, 4) if avg_salary > 0 else None,
