...
class BasketballReferenceCollector(BaseWebScrapingCollector):
    ...
    async def collect_projections(
        ...
        try:
            ...
            schedule_url = f"{self.config.base_url}/leaders/pts_per_g/{game_date.year}.html"
                            ...
                            if player_link:
                                ...
                                if ppg is not None and games is not None and games > 0:
                                    projection = PlayerProjection(
                                        player_id=player_id or f"br_{player_name.lower().replace(' ', '_')}",
                                        player_name=player_name,
                                        team=team,
                                        opponent="TBD",  # Would need to scrape schedule separately
                                        position=position,
                                        projected_points=ppg,
                                        game_date=game_date,
                                        source=self.config.source_name,
                                        last_updated=datetime.now(),
                                        confidence=0.7  # Season averages are moderately reliable
                                    )
            ...
            self.logger.info(f"Collected {len(projections)} projections from Basketball Reference")
            return projections
            
        except Exception as e:
            raise DataCollectionError(f"Failed to collect from Basketball Reference: {e}")
    ...
    async def get_available_dates(self, sport: SportType) -> List[date]:
        ...
        try:
            ...
            if soup.find('table', {'id': 'nba-stats'}) or soup.find('table', {'class': 'stats_table'}):
